...
전체 시스템 구조(각 기능이 내부적으로 어떻게 동작하는지)는 **[ARCHITECTURE.md](ARCHITECTURE.md)**
참고.
...
- **실시간 항공기 트래픽**: OpenSky Network 기반, 10초 주기 자동 갱신(레이어 on/off),
  제주항공(JJA)편 별도 구분
- **항로 제출 실적**: FOIS(국토부) 실제 제출 ATC 비행계획을 기간별로 수집·집계 — 구간별
  항로/기종/항공사 분석, NAVBLUE 항로DB 매칭 여부 표시, 특정 waypoint 통과 이력 검색
- **우회입항 시나리오 조회**: ATFM 우회입항 절차 대응용. 제약 waypoint 통과편을 실시간
  스케줄에서 조회하고, 과거 실적 기반으로 우회 waypoint를 자동 추천(항공사·예시편 근거
  포함, FIR 경계 우선순위)
- **관리자 페이지**: 근간 데이터(NAVDATA/항로DB/공항최저치) 업로드, 항로실적 아카이브
  수집 현황·거점공항 커버리지, 외부 데이터소스 헬스체크 — 상세는 ARCHITECTURE.md 참고
