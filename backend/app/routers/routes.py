...
def list_routes(
    ...
    corridor: Optional[str] = Query(None, description="Space-separated route string (e.g. 'ANRAT A326 DONVO') — routes touching ANY part of it (OR)"),
):
    routes = store.get_routes(origin=origin, destination=destination, fix=fix, fir=fir, corridor=corridor)
...
def route_geometry(
    ...
    fix: Optional[str] = Query(None, description="Comma-separated fix/airway names (AND)"),
    fir: Optional[str] = Query(None, description="FIR/UIR ICAO code — routes passing through it"),
    corridor: Optional[str] = Query(None, description="Space-separated route string — routes touching ANY part of it (OR)"),
    ...
    unfiltered = origin is None and destination is None and fix is None and fir is None and corridor is None and id_list is None
    if unfiltered and store.all_routes_geojson_cache is not None:
        return store.all_routes_geojson_cache

    routes = store.get_routes(
        origin=origin, destination=destination, fix=fix, fir=fir, corridor=corridor, ids=id_list
    )
